...
const Product = ({ product, onNavigator }) => {
  ...
  return (
    <div className="shop">
      <img src={product.Image} onClick={handleLinkToDetail} alt="" />
      <div className="info">
        <h3> {product.Name} </h3>
      ...
      <div className="shop-btn">
        <button onClick={handleClickAdd} className="btn1">
          ADD TO CART
        </button>
      </div>
    </div>
    ...
    const fetchData = async () => {
      if (keyword) {
        const resp = await axios.get(
          `http://localhost:8800/api/products?Keyword=${keyword}`
        );
          ...
          <div className="list-shop">
            {productList?.map((item, index) => (
              <Product
                key={index}
                product={item}
                onNavigator={(e) => handleNavigateDetail(e)}
              />
            ))}
          </div>
